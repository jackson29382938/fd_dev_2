...
def run_health_diagnostics() -> Dict[str, Any]:
    ...
    checks["java_runtime"] = {"available": java_available}
    if not java_available:
        warnings.append("Java runtime not found on PATH. UPS MaxiCode generation may fail.")

    for label, jar_path in {
        "zxing_core_jar": Path(ZXING_CORE_JAR_PATH),
        "zxing_javase_jar": Path(ZXING_JAVASE_JAR_PATH),
    }.items():
        if not _check_path(jar_path, label=label, issues=issues, checks=checks, required=False):
            warnings.append(f"MaxiCode helper jar missing: {label} ({jar_path})")
