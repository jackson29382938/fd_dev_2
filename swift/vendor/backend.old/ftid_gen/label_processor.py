...
def compose_label(
    ...
    if ftid_info:
        ...
        if "ups" in name.lower():
            from maxicode.decode_maxicode import OUTPUT_IMAGE, generate_maxicode_image

            maxicode_path = OUTPUT_IMAGE
            if os.path.exists(maxicode_path):
                os.remove(maxicode_path)

            generate_maxicode_image(maxicode_path, scale_override=maxicode_cfg.get("scale"))
