...
# --- Main Encoding Function with Platform-Specific Logic ---
def encode_maxicode(data, output_path, scale_override=None):
    """
    Encode data as MaxiCode using the preferred method for the current OS:
    1. Windows: Primary is TEC-IT online, secondary is Zint CLI.
    2. macOS/Linux: Primary is Zint Python wrapper, secondary is TEC-IT online.

    Note: The 'data' passed to this function must be the full combined string
          (e.g., Primary + '\u001D' + Secondary).

    Args:
        scale_override: Optional float to override the default Zint scale (2.0).
    """

    if not isinstance(data, str) or '\u001D' not in data:
        # A simple check to remind the caller about the required data format
        print("WARNING: Data may not be in the required 'Primary\\u001DSecondary' format for MaxiCode.")
    ...
    while True:
        try:
            if sys.platform == "win32":
                # ----------------------------------------------------
                # 1. WINDOWS PRIMARY: TEC-IT Online Method
                # ----------------------------------------------------
                try:
                    print("WINDOWS: Attempting primary method (TEC-IT Online)...")
                    if encode_maxicode_tec_it(data, output_path):
                        return True
                except RuntimeError as e:
                    print(f"WINDOWS: Primary online method failed ({e}). Falling back to Zint CLI...")

                    # 2. WINDOWS SECONDARY: Zint CLI Method
                    try:
                        if encode_maxicode_zint_cli(data, output_path):
                            print("WINDOWS: Successful fallback to Zint CLI.")
                            return True
                    except Exception as e_zint:
                        raise RuntimeError(f"WINDOWS: Both encoding methods failed. Last error (Zint CLI): {e_zint}")

            else: # macOS/Linux
                # ----------------------------------------------------
                # MACOS/LINUX: Try Zint first, fall back to TEC-IT Online
                # ----------------------------------------------------
                try:
                    if ZINT_AVAILABLE:
                        print("MACOS/LINUX: Attempting primary method (Zint Python Wrapper)...")
                        data = data.encode('utf-8') if isinstance(data, str) else data
                        data = data.decode('utf-8')

                        # Reduce data length
                        target_length = MAX_MAXICODE_LENGTH - 15
                        if len(data) > target_length:
                            print(f"Data exceeds {target_length} characters. Truncating.")
                            data = data[:target_length]
                        data = data.encode('utf-8')

                        sym = Symbol()
                        sym.symbology = Symbology.MAXICODE
                        sym.scale = scale_override if scale_override is not None else 2.0
                        sym.border_width = 0
                        sym.whitespace_width = 0
                        sym.outfile = output_path
                        sym.encode(data)
                        sym.print()

                        if os.path.exists(output_path):
                            print("MACOS/LINUX: Successful Zint Python wrapper.")
                            return True
                        else:
                            raise RuntimeError("Output file was not created by Zint wrapper.")
                    else:
                        # Zint not available, go directly to online fallback
                        raise RuntimeError("Zint library not available")

                except Exception as e:
                    print(f"MACOS/LINUX: Zint method failed ({e}). Using TEC-IT Online...")

                    # MACOS/LINUX FALLBACK: TEC-IT Online Method
                    try:
                        if encode_maxicode_tec_it(data, output_path):
                            print("MACOS/LINUX: Successful fallback to TEC-IT Online.")
                            return True
                    except Exception as e_tec_it:
                        raise RuntimeError(f"MACOS/LINUX: Both encoding methods failed. Last error (TEC-IT): {e_tec_it}")

        except Exception as e:
            # Original error handling logic for retries/abort
            ...
            # This line assumes a function 'get_user_choice' exists, which is missing here.
            # choice = get_user_choice("Enter your choice (1/2/3): ", ['1', '2', '3'])
            choice = input("Enter your choice (1/2/3): ").strip() # Using standard input for demonstration
...
def validate_config():
    ...
    # Check for standalone JAR first
    if not os.path.exists(ZXING_STANDALONE_JAR_PATH):
        # Check for separate JARs
        if not os.path.exists(ZXING_CORE_JAR_PATH):
            errors.append("ZXing Core JAR not found: core-3.5.0.jar")
        if not os.path.exists(ZXING_JAVASE_JAR_PATH):
            errors.append(f"ZXing JavaSE JAR not found: {ZXING_JAVASE_JAR_PATH}")
        if not os.path.exists(JCOMMANDER_JAR_PATH):
            warnings.append(f"JCommander JAR not found: {JCOMMANDER_JAR_PATH}")
